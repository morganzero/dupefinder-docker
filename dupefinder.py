...
def get_dupes(plex_section_name):
    ...
    while retries > 0:
        try:
            ...
            retries -= 1
            if retries == 0:
                log.exception("Timeout error while searching for duplicates in section %s", plex_section_name)
                print("Timeout error while searching for duplicates in section %s" % plex_section_name)
                exit(1)
            else:
                log.warning("Retrying search for duplicates in section %s", plex_section_name)
                time.sleep(5)
...
if __name__ == "__main__":
    print("""
   ___  __  _____  _____________  _____  _______
  / _ \/ / / / _ \/ __/ __/  _/ |/ / _ \/ __/ _ \ 
 / // / /_/ / ___/ _// _/_/ //    / // / _// , _/
/____/\____/_/  /___/_/ /___/_/|_/____/___/_/|_|
                                       by l3uddz
""")
    print("Initialized")
    process_later = {}
        ...
        for item in dupes:
            if item.type == 'episode':
                title = "%s - %02dx%02d - %s" % (
                    ...
                    int(item.parentIndex) if item.parentIndex is not None else 0,
